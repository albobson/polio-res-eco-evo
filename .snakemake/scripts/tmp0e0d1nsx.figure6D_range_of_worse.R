...
snakemake <- Snakemake(
    ...
    output = list('runs/ddt_logistic_mu_2e-05_2025-03-24/res/fig6/D.rds'),
    params = list('2025-03-24', c('logistic'), c(2e-05), "date" = '2025-03-24', "fit_func" = c('logistic'), "mu" = c(2e-05)),
    wildcards = list(),
    threads = 1,
    log = list(),
    resources = list('mem_mb', 'disk_mb', 'tmpdir', 'log_loc', 'mfree', 'cluster_time', 'disk_free', 'runtime', 'n_cores', "mem_mb" = '3G', "disk_mb" = '4G', "tmpdir" = '/tmp/12724535.1.feder-short.q', "log_loc" = './log_scr/', "mfree" = '3G', "cluster_time" = '0:00:10:00', "disk_free" = '4G', "runtime" = 10, "n_cores" = '1'),
    config = list(),
    rule = 'fig6D',
...
min_scale <- 1
max_scale <- 1/fit_func$prob_surv[1]

scale_range <- 10^log10(seq(min_scale, max_scale, length.out = 1000))
...
ggsave(paste0(filepath, "res/fig6/D.png"), func_plot, h = 22/3, w = 18/3, unit = "cm", bg = "transparent", dpi = 300)
ggsave(paste0(filepath, "res/fig6/D.pdf"), func_plot, h = 22/3, w = 18/3, unit = "cm", bg = "transparent", dpi = 300)
ggsave(paste0(filepath, "res/fig6/D.svg"), func_plot, h = 22/3, w = 18/3, unit = "cm", bg = "transparent", dpi = 300)
saveRDS(func_plot, file = paste0(filepath, "res/fig6/D.rds"))
