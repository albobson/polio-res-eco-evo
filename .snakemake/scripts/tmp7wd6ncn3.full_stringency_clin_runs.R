...
snakemake <- Snakemake(
    ...
    output = list('runs/ddt_logistic_mu_2e-05_2025-03-24/dat_gen/sims/all_full_stringency_trials.csv'),
    params = list('2025-03-24', c('logistic'), c(2e-05), "date" = '2025-03-24', "fit_func" = c('logistic'), "mu" = c(2e-05)),
    wildcards = list(),
    threads = 1,
    log = list(),
    resources = list('tmpdir', 'log_loc', 'mfree', 'cluster_time', 'mem_mb', 'disk_free', 'disk_mb', 'runtime', 'n_cores', "tmpdir" = 'C:\\Users\\meowy\\AppData\\Local\\Temp', "log_loc" = './log_scr/', "mfree" = '3G', "cluster_time" = '3:00:00:00', "mem_mb" = '3G', "disk_free" = '4G', "disk_mb" = '4G', "runtime" = 4320, "n_cores" = '20'),
    config = list(),
    rule = 'all_stringency_trials',
    bench_iteration = as.numeric(NA),
    scriptdir = 'C:\\Users\\meowy\\OneDrive\\Documents\\PhD\\dominant_drug_targets\\pipeline\\scr/sims',
    source = function(...){
        wd <- getwd()
...
# ## 70/93 were at 24 in the original trisal.
...
## Run a default of 2000 individuals across the range
n_sims <- 2000
...
scaled_vector <- function(og_probs, scaler) {
  ...
  new_vec <- ifelse(new_vec > 1, 1, new_vec)
...
min_scale <- 1
max_scale <- round(1/fit_func$prob_surv[1],0)

scale_range <- 10^seq(from = log10(min_scale), to = log10(max_scale), length.out = n_sims)
...
mixed_sims <- foreach(i = 1:length(scale_range), 
            ...
            seed_vec <- round(runif(n_sims, 1, 1000000))
...
write.csv(mixed_sims, paste0(filepath, "dat_gen/sims/all_full_stringency_trials.csv"), row.names = FALSE)
