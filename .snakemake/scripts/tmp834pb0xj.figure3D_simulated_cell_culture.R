...
snakemake <- Snakemake(
    ...
    threads = 1,
    log = list(),
    resources = list('mem_mb', 'disk_mb', 'tmpdir', 'log_loc', 'mfree', 'cluster_time', 'disk_free', 'runtime', 'n_cores', "mem_mb" = '3G', "disk_mb" = '4G', "tmpdir" = '/tmp/12666805.1.feder-short.q', "log_loc" = './log_scr/', "mfree" = '3G', "cluster_time" = '0:00:10:00', "disk_free" = '4G', "runtime" = 10, "n_cores" = '7'),
    config = list(),
    rule = 'fig3D',
...
my_p <- ggplot(my_data_clean, aes(x = moi_wt, y = sum_pfu, fill = sim)) +
  ...
  scale_fill_manual(values = c("Simulated" = "darkgrey",
                               "Cell Culture" = "black"),
                    label = c("Simulated",
                              "Cell Culture"),
                    limits = c("Simulated", "Cell Culture"),
                    breaks = c("Simulated", "Cell Culture")) +
  theme_classic() + 
  guides(fill = guide_legend(override.aes = list(fill = c("darkgrey", "black")))) +
  labs(fill = "Type") +
...
full_my_p <- plot_grid(my_p, NULL, tan_mois,
                      rel_heights = c(1.2, -0.05, 0.3),
                      ncol = 1, axis = "l", align = "v")



ggsave(paste0(filepath, "res/fig3/D.png"), full_my_p, h = 8, w = 8, bg = "transparent")
ggsave(paste0(filepath, "res/fig3/D.jpg"), full_my_p, h = 8, w = 8, bg = "transparent")
ggsave(paste0(filepath, "res/fig3/D.svg"), full_my_p, h = 8, w = 8, bg = "transparent", device = 'svg')
saveRDS(full_my_p, paste0(filepath, "res/fig3/D.rds"))