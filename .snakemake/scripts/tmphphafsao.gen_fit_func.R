...
snakemake <- Snakemake(
    ...
    threads = 1,
    log = list(),
    resources = list('mem_mb', 'disk_mb', 'disk_mib', 'tmpdir', 'log_loc', 'mfree', 'cluster_time', 'disk_free', 'runtime', 'n_cores', "mem_mb" = '3G', "disk_mb" = 1000, "disk_mib" = 954, "tmpdir" = '/tmp/12722194.1.feder-short.q', "log_loc" = './log_scr/', "mfree" = '3G', "cluster_time" = '0:00:10:00', "disk_free" = '2G', "runtime" = 60, "n_cores" = '1'),
    config = list(),
    rule = 'fit_func',
    bench_iteration = as.numeric(NA),
    scriptdir = '/net/feder/vol1/home/alexrob/projects/dominant_drug_targets/pipeline/scr/optimizations',
    source = function(...){
        wd <- getwd()
...
## Save this
dir.create(paste0(filepath, "dat_gen/"))
dir.create(paste0(filepath, "dat_gen/params"))
...
## Save in a dataframe
summ_fit_df <- data.frame(
  k = new_k,
  x0 = new_x0,
  converge_tolerance = summ_fit$convInfo$finTol,
  iterations = summ_fit$convInfo$finIter
)
...
write.csv(x = summ_fit_df, file = paste0(filepath, "dat_gen/params/logistic_fit_summary.csv"), row.names = F)
