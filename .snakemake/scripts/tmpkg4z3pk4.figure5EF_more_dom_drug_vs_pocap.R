...
snakemake <- Snakemake(
    input = list('runs/ddt_logistic_mu_2e-05_2025-03-24/dat_gen/params/optim_params.csv', 'runs/ddt_logistic_mu_2e-05_2025-03-24/dat_gen/params/logistic_fitness_function.csv', 'runs/ddt_logistic_mu_2e-05_2025-03-24/dat_gen/sims/full_dominance_trials.csv'),
    output = list('runs/ddt_logistic_mu_2e-05_2025-03-24/res/fig5/E.rds', 'runs/ddt_logistic_mu_2e-05_2025-03-24/res/fig5/F.rds'),
    params = list('2025-03-24', c('logistic'), c(2e-05), "date" = '2025-03-24', "fit_func" = c('logistic'), "mu" = c(2e-05)),
    wildcards = list(),
    threads = 1,
    log = list(),
    resources = list('mem_mb', 'disk_mb', 'tmpdir', 'log_loc', 'mfree', 'cluster_time', 'disk_free', 'runtime', 'n_cores', "mem_mb" = '3G', "disk_mb" = '4G', "tmpdir" = '/tmp/12724410.1.feder-short.q', "log_loc" = './log_scr/', "mfree" = '3G', "cluster_time" = '0:00:10:00', "disk_free" = '4G', "runtime" = 10, "n_cores" = '1'),
    config = list(),
    rule = 'fig5EF',
...
plot5e <- ggplot(sims_id, aes(x = dom_val, 
  ...
  geom_line(aes(y = clear_rolling_mean), color = "black", linewidth = 1) +
  ...
  theme_light() +
  xlab("Susceptible Dominance") + ylab("Clearance Date (DPI)") +
  # scale_x_log10(
    # breaks=10^(-5:0),
...
ggsave(paste0(filepath, "res/fig5/E.png"), plot5e, h = 22/3, w = 18/3, unit = "cm", bg = "transparent", dpi = 300)
ggsave(paste0(filepath, "res/fig5/E.pdf"), plot5e, h = 22/3, w = 18/3, unit = "cm", bg = "transparent", dpi = 300)
ggsave(paste0(filepath, "res/fig5/E.svg"), plot5e, h = 22/3, w = 18/3, unit = "cm", bg = "transparent", dpi = 300)
saveRDS(plot5e, file = paste0(filepath, "res/fig5/E.rds"))
...
plot5f <- ggplot(sims_id, aes(x = dom_val, 
  ...
  theme_light() +
  xlab("Susceptible Dominance") + ylab("Sum Total Viral Load") +
  scale_x_log10(
    # breaks=10^(-5:0),
    # limits = c((min(sims_id$wt_rep_abil)-min(sims_id$wt_rep_abil)*0.1), 1.5),
    # labels = sapply(-5:0,function(j){parse(text = sprintf("10^%d",j))})
  ...
  theme(
    ...
    # legend.position = "inside",
    # legend.position.inside = c(0.35, 0.94), ## Upper left
    legend.position = c(0.89, 0.15), ## bottom right
...
ggsave(paste0(filepath, "res/fig5/F.png"), plot5f, h = 22/3, w = 18/3, unit = "cm", bg = "transparent", dpi = 300)
ggsave(paste0(filepath, "res/fig5/F.pdf"), plot5f, h = 22/3, w = 18/3, unit = "cm", bg = "transparent", dpi = 300)
ggsave(paste0(filepath, "res/fig5/F.svg"), plot5f, h = 22/3, w = 18/3, unit = "cm", bg = "transparent", dpi = 300)
saveRDS(plot5f, file = paste0(filepath, "res/fig5/F.rds"))
