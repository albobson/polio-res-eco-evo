...
snakemake <- Snakemake(
    ...
    threads = 1,
    log = list(),
    resources = list('mem_mb', 'disk_mb', 'tmpdir', 'log_loc', 'mfree', 'cluster_time', 'disk_free', 'runtime', 'n_cores', "mem_mb" = '3G', "disk_mb" = '4G', "tmpdir" = '/tmp/12724532.1.feder-short.q', "log_loc" = './log_scr/', "mfree" = '3G', "cluster_time" = '0:00:10:00', "disk_free" = '4G', "runtime" = 10, "n_cores" = '1'),
    config = list(),
    rule = 'fig6A',
...
## This will plot the less stringent drug alongside pocapavir
...
full_df <- rbind(fit_func, scaled_fit_func)
...
func_plot <- ggplot() +
  ...
  geom_line(data = scaled_fit_func,
            mapping = aes(group = fit_type,
                x = subunits, 
                y = prob_surv, 
                color = fit_type, 
  ...
  theme_light() +
  scale_color_manual(labels = c(paste0(scale, "x Less Stringent"), "Pocapavir"),
...
ggsave(paste0(filepath, "res/fig6/A.png"), func_plot, h = 22/3, w = 18/2, unit = "cm", bg = "transparent", dpi = 300)
ggsave(paste0(filepath, "res/fig6/A.pdf"), func_plot, h = 22/3, w = 18/2, unit = "cm", bg = "transparent", dpi = 300)
ggsave(paste0(filepath, "res/fig6/A.svg"), func_plot, h = 22/3, w = 18/2, unit = "cm", bg = "transparent", dpi = 300)
saveRDS(func_plot, file = paste0(filepath, "res/fig6/A.rds"))
