stoch_polv_iter <- function(n = 1,
  ...
  for (t in 1:n) {
    ...
    ## Summarize the output of each
    ## Total
    inf_w_n_v <- as.vector(table(tot_v))
    
    ## Resistant
    tot_inf_w_n_r <- as.vector(table(res_vec))
    ## Padding out the cells that were not represented here
    tot_inf_w_n_r <- c(tot_inf_w_n_r, rep(0, length(inf_w_n_v) - length(tot_inf_w_n_r)))
    ...
    df <- data.frame(inf_w_n_v, tot_inf_w_n_r)
...
# 
    ...
    df$tot_v_in_cells <- mapply(f_bursts, df$inf_w_n_v)
