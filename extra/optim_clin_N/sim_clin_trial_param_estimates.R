...
## Cell population
init_c_pop <- as.numeric(snakemake@wildcards[["c_pop"]])
...
saveRDS(optim_clin_trial_params, file = paste0(filepath, "_params_c_pop_", init_c_pop, ".rds"))

## Save the values that the optim function iterated over
saveRDS(vals, file = paste0(filepath, "_vals_params_c_pop_", init_c_pop, ".rds"))
...
write.csv(optim_df, file = paste0(filepath, "_params_c_pop_", init_c_pop, ".csv"), row.names = F)