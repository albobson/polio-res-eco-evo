## Plotting the trajectories of the specific trials
...
trial_all <- read.csv(paste0("250810_range_of_cpop/c_pop_trial_data_first_run.csv"))

## Things to filter by
c_pop_filter_by = 10^4

init_wt_filter_by = 10

unique(trial_all$init_wt*trial_all$c_pop)

trial <- trial_all %>%
  filter(round(c_pop, 5) == round(c_pop_filter_by, 5), 
         round(init_wt, 5) == round(init_wt_filter_by/c_pop_filter_by, 5)
         )
...
sup_plot4 <- ggplot(simsclean, aes(x = time, y = surv_pfu/c_pop, color = type, group = interaction(id, type))) +
  ...
  ylab("Total viral population size (genomes)") + 
  ...
  ggtitle("cell population: 10^4.5, init V: 10") +
  facet_grid(rows = vars(earlylate),
             cols = vars(time_to_pocap)) 
