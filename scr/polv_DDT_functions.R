...
determ_polv <-
  function(n = 1,
    ...
    for (t in 1:n) {
      ...
      vpc <- qbinom(0.9999999999999, v_pop_tot, 1/c_pop)
      if (vpc < 1) {
        vpc <- 1
      }
      ...
      ## Create a function for dbinom() which can accept fractional values for
      ## size
      ## X corresponding to the number of successes
      ## y corresponding to size (number of trials) (can be fractional)
      ## z corresponding to the probability
      dbinom_frac <- function(x, y, z) {
        whole_n <- floor(y)
        frac <- y - whole_n
        
        low_res <- (1 - frac) * dbinom(x, whole_n, z)
        high_res <- frac * dbinom(x, (whole_n+1), z)
        
        tot_prob <- low_res + high_res
        
        return(tot_prob)
      }
      
      df_res$prob_inf_w_n_v <- 
        dbinom_frac(df_res$viruses_per_cell, c(v_pop_tot), 1/c_pop)
      
      ## Resistant viruses
      df_res$prob_inf_w_n_r <- 
        dbinom_frac(df_res$res_per_cell, c(v_pop_mut), 1/c_pop)
      
      ## Exact number of viruses in the cell
      df_res$prob_inf_w_exact <- 
        ## Prob of resistant virus being in cell
        dbinom_frac(df_res$res_per_cell, c(v_pop_mut), 1/c_pop) *
        ## Prob of susceptible virus being in cell
        dbinom_frac(df_res$viruses_per_cell - df_res$res_per_cell, c(v_pop_wt), 1/c_pop)
      
      ## Susceptible
      ## total number of viruses
      df_sus$prob_inf_w_n_v <- 
        dbinom_frac(df_sus$viruses_per_cell, c(v_pop_tot), 1/c_pop)
      
      ## Resistant viruses
      df_sus$prob_inf_w_n_r <- 
        dbinom_frac(df_sus$sus_per_cell, c(v_pop_wt), 1/c_pop)
      
      ## Exact number of viruses in the cell
      df_sus$prob_inf_w_exact <- 
        ## Prob of resistant virus being in cell
        dbinom_frac(df_sus$sus_per_cell, c(v_pop_wt), 1/c_pop) *
        ## Prob of susceptible virus being in cell
        dbinom_frac(df_sus$viruses_per_cell - df_sus$sus_per_cell, c(v_pop_mut), 1/c_pop)
      ...
      ## dbinom_frac(df_res$res_per_cell, df_res$viruses_per_cell, v_prop) = The
      ...
      # df_res$prob_inf_w_n_r <- 
      #   dbinom_frac(df_res$res_per_cell, df_res$viruses_per_cell, v_prop)
      
      ## dbinom_frac(df_res$subs, subunits,
      ...
      ## Updated code below
      df_res$tot_prob <- df_res$prob_inf_w_exact *
        df_res$prob_prog_w_n_res_subs * df_res$prob_res_genome
      ...
      # 
      # df_sus$prob_inf_w_n_r <-
      #   dbinom_frac(df_sus$sus_per_cell, df_sus$viruses_per_cell, (1 - v_prop))
      df_sus$prob_prog_w_n_sus_subs <-
        dbinom_frac(df_sus$subs, subunits,
      ...
      df_sus$tot_prob <- df_sus$prob_inf_w_exact *
        df_sus$prob_prog_w_n_sus_subs * df_sus$prob_sus_genome
      
      
      ## Finding the number of uninfected cells
      # dist <- dpois(0:vpc, moi_tot)
      dist <- dbinom_frac(0, c(v_pop_wt), 1/c_pop) * 
        dbinom_frac(0, c(v_pop_mut), 1/c_pop)
...
#       ## total number of viruses
#       df_res$prob_inf_w_n_v <- 
#         dbinom(df_res$viruses_per_cell, as.integer(v_pop_tot), 1/c_pop)
#       
#       ## Resistant viruses
#       df_res$prob_inf_w_n_r <- 
#         dbinom(df_res$res_per_cell, as.integer(v_pop_mut), 1/c_pop)
#       
#       ## Exact number of viruses in the cell
#       df_res$prob_inf_w_exact <- 
#         ## Prob of resistant virus being in cell
#         dbinom(df_res$res_per_cell, as.integer(v_pop_mut), 1/c_pop) *
#         ## Prob of susceptible virus being in cell
#         dbinom(df_res$viruses_per_cell - df_res$res_per_cell, as.integer(v_pop_wt), 1/c_pop)
#       
#       ## Susceptible
#       ## total number of viruses
#       df_sus$prob_inf_w_n_v <- 
#         dbinom(df_sus$viruses_per_cell, as.integer(v_pop_tot), 1/c_pop)
#       
#       ## Resistant viruses
#       df_sus$prob_inf_w_n_r <- 
#         dbinom(df_sus$sus_per_cell, as.integer(v_pop_wt), 1/c_pop)
#       
#       ## Exact number of viruses in the cell
#       df_sus$prob_inf_w_exact <- 
#         ## Prob of resistant virus being in cell
#         dbinom(df_sus$sus_per_cell, as.integer(v_pop_wt), 1/c_pop) *
#         ## Prob of susceptible virus being in cell
#         dbinom(df_sus$viruses_per_cell - df_sus$sus_per_cell, as.integer(v_pop_mut), 1/c_pop)
#       
#       
#       ###### Resistant
#       ## dpois(df_res$viruses_per_cell, moi_tot) = the probability of a cell
...
#       ## `viruses_per_cell` viruses, `res_per_cell` of them being resistant
#       df_res$prob_prog_w_n_res_subs <- 
#         dbinom(df_res$subs, as.integer(subunits), 
#                (df_res$res_per_cell/df_res$viruses_per_cell))
#       
#       ## (df_res$res_per_cell/df_res$viruses_per_cell) + ((1 -
#       ## (df_res$res_per_cell / df_res$viruses_per_cell)) / bg_mutat) -
...
#       df_sus$prob_prog_w_n_sus_subs <-
#         dbinom(df_sus$subs, as.integer(subunits),
#                (1-(df_sus$sus_per_cell/df_sus$viruses_per_cell)))
#       
#       df_sus$prob_sus_genome <- 
#         (df_sus$sus_per_cell/df_sus$viruses_per_cell) +
...
#       ## Finding the number of uninfected cells
#       # dist <- dpois(0:vpc, moi_tot)
#       dist <- dbinom(0, floor(v_pop_wt), 1/c_pop) * 
#         dbinom(0, floor(v_pop_mut), 1/c_pop)
#       cell_dist <- dist * c_pop
#       
#       #### Total number of viruses #### 
#       ## Now, multiply the probabilities by the total number of cells, the burst
...
stoch_polv <- function(n = 1,
  ...
  for (t in 1:n) { 
    ...
    # print(paste0("moi wt: ", moi_wt, "moi mut: ", moi_mut))
    ...
    ## Find how many viruses we have:
    wt_v <- moi_wt*c_pop
    mut_v <- moi_mut*c_pop
    
    # # print(paste0("wt ", wt_v, " mut ", mut_v))
    # print(paste0("Line 634"))
    ## Check that we have viruses
    if(sum(floor(wt_v), floor(mut_v)) < 1) break
    
    # print(paste0("wt_v :", wt_v))
    
    # print(paste0("mut_v :", mut_v))
    
    ## Pull the viruses across these cell populations
    ## Create a function to iterate over if the population sizes are too large
    # f_inf_multinom_large <- function(x, y, max_chunk) {
    ...
    #     inf_vec <- as.vector(rmultinom(1, floor(x), prob = rep(1, y)))
    #   } 
    #   ## If there are more pulls than the computer can handle, run in batches
    #   if(chunks > 1) {
    #     ## floor() rounds down to the nearest integer
    #     new_temp <- rmultinom(n = floor(chunks), size = max_chunk, prob = rep(1, y))
    ...
    #     new_remand <- rmultinom(n = 1, size = floor(max_chunk*remainder), prob = rep(1, y))
    #     inf_vec <- new_remand+new_viruses
    #   }
    #   return(unlist(inf_vec))
    # }
    f_inf_multinom_large <- function(x, y, max_chunk = .Machine$integer.max) {
      # if (y <= 0) return(integer(y))
      
      total <- x
      full_batches <- total %/% max_chunk
      remainder <- total %% max_chunk
      
      inf_vec <- rep(0, y)
      
      # Do batches one at a time to avoid giant matrices
      if (full_batches > 0) {
        for (i in 1:full_batches) {
          inf_vec <- inf_vec + rmultinom(1, size = max_chunk, prob = rep(1, y))
        }
      }
      
      if (remainder > 0) {
        inf_vec <- inf_vec + rmultinom(1, size = floor(remainder), prob = rep(1, y))
      }
    ...
    # print(paste0("Line 1129"))
    
    res_vec <- mapply(f_inf_multinom_large, 
                                    x = mut_v, 
                                    y = c_pop,
                                    max_chunk = .Machine$integer.max)
    
    # print(paste0("Line 1136"))
    
    sus_vec <- mapply(f_inf_multinom_large, 
                      x = wt_v, 
                      y = c_pop,
                      max_chunk = .Machine$integer.max)
    
    # print(paste0("Line 1143"))
    
    # res_vec <- as.vector(rmultinom(1, mut_v, prob = rep(1, c_pop)))
    # sus_vec <- as.vector(rmultinom(1, wt_v, prob = rep(1, c_pop)))
    # print(paste0("Line 641"))
    ## Create a vector of total virus per cell
    tot_v <- res_vec + sus_vec
    # print(paste0("Line 1174"))
    ## Dataframe for everything
    df <- data.frame(res_vec, tot_v, sus_vec)
    # print(paste0("Line 1177"))
    ## Create a table for the options
    # df <- table(df$tot_v, df$res_vec)
    df <- df %>% count(tot_v, res_vec)
    # print(paste0("Line 1180"))
    ## Turn this into a data.frame
    df <- as.data.frame(df)
    # print(paste0("Line 1183"))
    ## Change the column names
    colnames(df) <- c("viruses_per_cell", "res_per_cell", "tot_inf_w_n_r")
    # print(paste0("Line 1186"))
    ## Make sure everything is being read as a number
    df$viruses_per_cell <- as.numeric(as.character(df$viruses_per_cell))
    df$res_per_cell <- as.numeric(as.character(df$res_per_cell))
    # print(paste0("Line 1190"))
    ## Sort by the v_tot to make things easier to read
    df <- arrange(df, viruses_per_cell)
    # print(paste0("Line 1193"))
    ## Remove any sections where there were 0 cells
    df <- subset(df, tot_inf_w_n_r > 0)
    # print(paste0("Line 1196"))
    ## We can now remove the 0s from our dataframe, since they have no viruses
    df_0s <- df[1,] ## Saving for later, just in case desired.
    
    df <- df[-1,]
    # print(paste0("Line 671"))
    # ## Create a "viruses per cell" column
    # df$viruses_per_cell <- 1:length(df$inf_w_n_v)
    # 
    ...
    df$tot_v_in_cells <- mapply(f_bursts, floor(df$tot_inf_w_n_r))
    # print(paste0("Line 705"))
    ...
    df$tot_r_b4_mut <- mapply(f_gen_res, x = as.numeric(df$tot_v_in_cells),
                              y = floor(df$res_per_cell),
                              z = floor(df$viruses_per_cell))
    
    df$tot_s_b4_mut <- df$tot_v_in_cells - df$tot_r_b4_mut
    # print(paste0("Line 718"))
    ...
    # print(paste0("Line 743"))
    # browser()
    ## Make sure there are still viruses in the system
    if(sum(floor(df$tot_r), floor(df$tot_s)) < 1) break
    ...
    # print(paste0("Line 833"))
    ## Recording the number of cells that did not get infected
    cell_surv <- as.vector(df_0s$tot_inf_w_n_r)
...
plot_traj_polv <- function(df, stoch = FALSE, plot_title = TRUE, title_text) {
  ...
  if(stoch == FALSE) {
    s <- ggplot(df, aes(x = time, y = surv_pfu, color = type)) +
      ...
      if(plot_title) {ggtitle(title_text)} +
