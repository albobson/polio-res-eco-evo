################ Figure X - Change in resistance by weakness ###################

## Reason:

## This script will plot the clinical trial trajectories for the four weaker
## runs in main text Figure 5.

## Figure dimensions: 11, 9
...
## This figure's size
#'*################### change this when I have the correct figure number#######*
...
trial <- read.csv(paste0(filepath, "dat_gen/sims/full_stringency_trials.csv"))
...
simsclean <- trial %>% 
  group_by(id, fold_strin) %>% 
  mutate(earlylate = ifelse(max(time) >= early_placebo, "Late", "Early"))



sup_plot <- ggplot(simsclean, aes(x = time/3, y = moi_type, color = type, group = interaction(id, type))) +
  ...
  theme(
    ...
    legend.position.inside = c(0.95, 0.955),
  ...
  facet_grid(cols = vars(fold_strin))

suppressWarnings(dir.create(paste0(filepath, "res/")))
suppressWarnings(dir.create(paste0(filepath, "res/sup/")))
suppressWarnings(dir.create(paste0(filepath, "res/sup/sfigXX")))

## Save
ggsave(paste0(filepath, "res/sup/sfigXX/sfigXX.png"), sup_plot, h = height, w = width, units = "cm", bg = "transparent", dpi = 1200)
ggsave(paste0(filepath, "res/sup/sfigXX/sfigXX.pdf"), sup_plot, h = height, w = width, units = "cm", bg = "transparent", dpi = 1200)
saveRDS(sup_plot, file = paste0(filepath, "res/sup/sfigXX/sfigXX.rds"))
