...
full_p <- ggplot(full_df, aes(x = time, y = moi_type, color = type)) +
  ...
  theme(
    ...
    legend.position.inside = c(0.905,0.085),
  ...
  facet_grid(rows = vars(clear))




## Save
ggsave(full_p, file = paste0(filepath, "res/fig4/GH.png"), h = height/2, w = width/3, units = "cm", bg = "transparent", dpi = 1200)
ggsave(full_p, file = paste0(filepath, "res/fig4/GH.pdf"), h = height/2, w = width/3, units = "cm", bg = "transparent", dpi = 1200)
ggsave(full_p, file = paste0(filepath, "res/fig4/GH.svg"), h = height/2, w = width/3, units = "cm", bg = "transparent", dpi = 1200, device = 'svg')
saveRDS(full_p, paste0(filepath, "res/fig4/GH.rds"))
